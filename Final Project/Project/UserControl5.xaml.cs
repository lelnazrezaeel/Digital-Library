...
namespace Project
{
    ...
    /// </summary>
    public partial class UserControl5 : System.Windows.Controls.UserControl
    {
        public string ImageFile = null;
        User user;
        public UserControl5(string name)
        {
            user = DataBaseManager.SearchInfo(name);
            InitializeComponent();
        ...
        {
            User user1 = new User(txtName.Text,txtEmail.Text,txtPhoneNumber.Text,  txtPassword.Password, 0, ImageFile,DateTime.Now);
            if (ValidateFields() && txtPassword.Password == user1.Password)
            {
                DataBaseManager.updateUser(user1);
